...
class VerseDetailPage extends StatefulWidget {
  ...
  const VerseDetailPage({super.key, required this.verse, required this.chapterNum, required this.verseNum, required this.color});
        ...
        child: Column(children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              gradient: RadialGradient(center: Alignment.topLeft, radius: 2, colors: [widget.color.withOpacity(0.2), const Color(0xFF2D1200)]),
              border: Border.all(color: widget.color.withOpacity(0.4), width: 1.5),
            ),
            child: Column(children: [
              Text('ॐ', style: TextStyle(fontSize: 32, color: widget.color)),
              const SizedBox(height: 16),
              Text(widget.verse['sanskrit']!, style: const TextStyle(color: Color(0xFFFFD700), fontSize: 18, height: 1.8, fontStyle: FontStyle.italic), textAlign: TextAlign.center),
            ]),
          ),
          const SizedBox(height: 20),
          _buildSection(title: 'अहिराणी अनुवाद', icon: '🗣️', content: widget.verse['ahirani']!, color: widget.color, isActive: _showTransliteration, onToggle: () => setState(() => _showTransliteration = !_showTransliteration)),
          const SizedBox(height: 16),
          _buildSection(title: 'अर्थ (मराठी)', icon: '📖', content: widget.verse['meaning']!, color: const Color(0xFF00C9A7), isActive: _showMeaning, onToggle: () => setState(() => _showMeaning = !_showMeaning)),
        ]),
      ),
    );
  }

  Widget _buildSection({required String title, required String icon, required String content, required Color color, required bool isActive, required VoidCallback onToggle}) {
    return Container(
      decoration: BoxDecoration(borderRadius: BorderRadius.circular(14), color: const Color(0xFF2D1200), border: Border.all(color: color.withOpacity(0.3))),
      child: Column(children: [
        GestureDetector(
          onTap: onToggle,
          child: Padding(
            ...
            child: Row(children: [
              ...
              const Spacer(),
              Icon(isActive ? Icons.keyboard_arrow_up : Icons.keyboard_arrow_down, color: color),
            ]),
          ),
        ),
        if (isActive)
          Padding(
            ...
            child: Column(children: [
              ...
              Text(content, style: const TextStyle(color: Color(0xFFDDC08A), fontSize: 15, height: 1.7)),
            ]),
          ),
      ]),
    );
